...
class OrderConversation extends Conversation
{
    ...
    public function askLink(Nutgram $bot): void
    {
        ...
        $harga  = number_format((float)$found['rate'] * $markup * 1000, 0, ',', '.');
    ...
    public function confirmOrder(Nutgram $bot): void
    {
        ...
        $totalHarga = ceil(($hargaPer1000 / 1000) * $qty);
        $totalRupiah = number_format($totalHarga * 1000, 0, ',', '.');
    ...
    public function processPayment(Nutgram $bot): void
    {
        ...
        $markup       = (float) NuestoreSetting::get('global_markup_multiplier', 2.0);
        $hargaPer1000 = (float)$this->selectedService['rate'] * $markup;
        $totalHarga   = (int) ceil(($hargaPer1000 / 1000) * $this->quantity) * 1000;
        $modalCost    = (float)$this->selectedService['rate'] / 1000 * $this->quantity * 1000;
